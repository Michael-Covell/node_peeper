...
def draw_multipartite_graph(g,communities):
    lengths = [len(community) for community in communities]
    print('communities = ', communities)
    print('community lengths = ',lengths)
    nodelist = [item for list in communities for item in list]
    colors = ('gold','cyan','silver','salmon','beige','wheat','limegreen','darkorange','violet','orange','green','purple')
    list_of_lists = [lengths[i]*[colors[i]] for i in range(len(lengths))]
    node_colors = [x for l in list_of_lists for x in l]
    nx.draw(g,
            with_labels=True,
            nodelist=nodelist,
            node_color=node_colors),plt.show();